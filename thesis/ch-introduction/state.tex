...
In this work, we investigate the properties of correlated phases in \ac{TMD} nanoribbons.
...
On the other direction, we use \acp{OBC}, effectively considering zigzag edges (Fig. (\ref{fig:nanoribbons}), left).

\begin{figure}[H]
\centering
\includegraphics[scale = 0.28]{Introduction/nanoribbons}
...
\vspace{-0.5cm}
\centering
\includegraphics[scale = 0.36]{Introduction/grapheneNanoribbons.jpg}
...
In fact, a mean field solution of the Hubbard model for a graphene nanoribbon shows that magnetic moments are localized at the edges \cite{yazyev_emergence_2010} (Fig. (\ref{fig:nanoribbons}), right).
