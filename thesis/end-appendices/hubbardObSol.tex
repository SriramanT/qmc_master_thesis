...
For example, for the \acs{TMD} nanoribbon of chapter \ref{cap:applications}, at iteration $I$ we have that
\begin{equation}
\begin{split}
\Omega_{\text{MF}, I} &= -\frac{1}{\beta} \sum_{q, \nu, \sigma} \ln \bigg( 1 + e^{-\beta [\varepsilon_{ \nu, \sigma} (q) ]_I} \bigg) \\
\left\langle \mathcal{H} - \mathcal{H}_{\text{MF}} \right\rangle_{\text{MF}, I} &= U N_x \bigg( \sum_{\mu} \left\langle n_{\mu, \uparrow} \right\rangle_I \left\langle n_{\mu, \downarrow} \right\rangle_I - \sum_{\mu, \sigma} \left\langle n_{\mu, \sigma} \right\rangle_{I - 1} \left\langle n_{\mu, -\sigma} \right\rangle_I \bigg)
\end{split}
\end{equation}
...
In this case, we consider a \acs{TMD} nanoribbon with $N_y = 16$, at inverse temperature $\beta | t_0 | = 2 $, at $U = 20 | t_0 |$, where $U$ is an on-site interaction that only occurs if the electrons occupy the same $d$-orbital.
Thus, only double occupancy of a site \emph{and an orbital} is penalized.

\begin{figure}[H]
\centering
\includegraphics[scale=0.55]{Applications/minimizationExample.png}
	\caption[Example of the minimization of the grandpotential functional.]{Example of the minimization of the grandpotential functional per particle in units of $| t_0 |$, $\Omega  / N$, for a \acs{TMD} nanoribbon of width $N_y = 16$, at inverse temperature $\beta | t_0 | = 2 $, at $U = 20 | t_0 |$.
	$N$ is the total number of sites in the system. \label{fig:grandPotMin}}
\end{figure}